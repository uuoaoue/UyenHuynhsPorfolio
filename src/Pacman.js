...
// pacman & ghost do not move at the begining of the game until you make the first move
  ...
  // set pacman rotation based on direction
  Rotation = {
    right: 0,
    down: 1,
    left: 2,
  ...
  draw(ctx, pause, enemies) {
    ...
    ctx.save();
    // rotate canvas then restore it back
  ...
  #move() {
    ...
    if (
      ...
      // stop animation if collide into wall
      this.pacmanAnimationTimer = null;
      this.pacmanImageIndex = 1;
      return;
    }
    // trigger pacman animation
    else if (
      this.currentMovingDirection != null &&
      this.pacmanAnimationTimer == null
    ) {
  ...
  #eatPowerDot() {
    if (this.tileMap.eatPowerDot(this.x, this.y) ) {
      // the ghost will turn blue + sound
